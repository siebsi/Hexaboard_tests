...
#datadir = "C:\Users\Administrator\Desktop\Hexaboard_Probecard_DAQ"
...
def run_test():
    ...
    if testswitches['IV_curve']==True:
        ...
        ivName=str(glb_options['outputCommonPath']) + "sensor"+str(glb_options['moduleNumber'])+'/IV_LOG/'
        ivName='Sensor'+str(glb_options['moduleNumber'])+"_"+str(the_time.day)+"-"+str(the_time.month)+"-"+str(the_time.year)+"_"+str(the_time.hour)+"-"+str(the_time.minute)+"-"+str(the_time.second)
        ivName=ivName+".pdf"
        plt.savefig(ivName)
        #plt.show()
        plt.close(1)
        ivName=str(glb_options['outputCommonPath']) + "module"+str(glb_options['moduleNumber'])+'/IV_LOG/'
        ivName=ivName+'Sensor'+str(glb_options['moduleNumber'])+"_"+str(the_time.day)+"-"+str(the_time.month)+"-"+str(the_time.year)+"_"+str(the_time.hour)+"-"+str(the_time.minute)+"-"+str(the_time.second)
        ivName=ivName+".csv"
        data_folder=Path(ivName)
        csvfile = open(data_folder, "w")
        ...
        secondrow='Sensor_Number:'+str(glb_options['moduleNumber'])
    ...
    if (int(daq_options['nEvent'])>0):
        for hv in daq_options['hv']:
            ...
            if options.dataNotSaved==False:
                while True:
                    rawFileName=str(glb_options['outputCommonPath']) + "sensor"+str(glb_options['moduleNumber'])+'\\Data\\'
                    rawFileName=rawFileName+'Sensor'+str(glb_options['moduleNumber'])+"_"+str(the_time.day)+"-"+str(the_time.month)+"-"+str(the_time.year)+"_"+str(the_time.hour)+"-"+str(the_time.minute)+"-"+str(the_time.second)
                    ...
                    if os.path.exists(rawFileName): #to avoid overwriting
                        ...
                        if glb_options['storeYamlFile']==True:
                            yamlFileName=str(glb_options['outputCommonPath']) + "sensor"+str(glb_options['moduleNumber'])+'\\ConfigOutput\\'
                            yamlFileName=yamlFileName+'Sensor'+str(glb_options['moduleNumber'])+"_"+str(the_time.day)+"-"+str(the_time.month)+"-"+str(the_time.year)+"_"+str(the_time.hour)+"-"+str(the_time.minute)+"-"+str(the_time.second)
